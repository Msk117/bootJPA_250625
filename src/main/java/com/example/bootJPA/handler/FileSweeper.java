...
public class FileSweeper {
    /* 매일 정해진 시간에 스케줄러를 실행
    *  매일 등록된 파일과 (DB) 해당 일자의 폴더의 파일과 일치하는지 비교
    *  일치하는 파일은(DB = file) 남기고, (DB != file) DB에 없는 파일은 삭제
    *  직접 DB 에서 조회 => fileRepository 연결
    * */

    private final FileRepository fileRepository;
    private final String BASE_PATH = "D:\\web_0226_kms\\_myProject\\_java\\_fileUpload\\";

    // cron 방식 = 초 분 시 일 월 요일 년도(년도만 생략가능)
    ...
    public void fileSweeper(){
        ...
        // 파일경로 + 파일명을 붙인 실제 파일 리스트 생성
         // D:\web_0226_kms\_myProject\_java\_fileUpload\2025\06\26\ uuid_파일명
        ...
        // 실제 저장되어 있는 파일 목록과, DB의 파일을 비교하여 DB에 없는 파일은 삭제진행
        // 오늘 날짜의 파일 경로 설정
        LocalDate now = LocalDate.now();
        String today = now.toString();
        today = today.replace("-", java.io.File.separator);

        // D:\web_0226_kms\_myProject\_java\_fileUpload\2025\06\26
